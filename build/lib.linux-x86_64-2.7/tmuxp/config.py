...
config_dir = os.getenv('TMUXP_DIR', os.path.expanduser('~/.tmuxp/'))
tmuxinator_config_dir = os.path.expanduser('~/.tmuxinator/')
teamocil_config_dir = os.path.expanduser('~/.teamocil/')


class TmuxpConfig(object):
    """
    tmuxp global config
    """

    default_filename = os.getenv('TMUXP_CONFIG',
        os.path.join(config_dir, ".tmuxp.conf"))

    def __init__(self, filename=None):
        self.config = ConfigParser.ConfigParser(allow_no_value=True)
        self.filename = filename or TmuxpConfig.default_filename
        self.change = False
        ...
        try:
            try:
                if self.types[key] == bool:
                    value = self.config.getboolean('global', key)
                elif self.types[key] == int:
                    value = self.config.getint('global', key)
                else:
                    value = self.config.get('global', key)
            except ValueError:
                sys.exit("Config value error: '%s'" % key)
...
def expand(sconf, cwd=None):
    ...
    if 'start_directory' in sconf:
        ...
        if start_path and any(start_path.startswith(a) for a in ['.', './']):
