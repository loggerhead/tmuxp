...
from .config import TmuxpConfig
from .config import config_dir, tmuxinator_config_dir, teamocil_config_dir
...
def startup(dirname=None):
    ...
    tmuxp_config = TmuxpConfig()

    if tmuxp_config['save_to_dir'] is None:
        tmuxp_config['save_to_dir'] = config_dir
    else:
...
def command_freeze(args):
    ...
    if not session_name and os.getenv('TMUX'):
        session_name = os.getenv('TMUX').rsplit(',', 1).pop()
    ctext = ' '.join(session_name)
    ...
    if tmuxp_config['format'] is None:
        tmuxp_config['format'] = prompt_choices(
            'Convert to',
            choices=['yaml', 'json'],
            default='yaml'
        )
    config_format = tmuxp_config['format']
    ...
    buf.write(newconfig)
    buf.close()
    tmuxp_config['last_save_file'] = dest
...
def get_parser():
    ...
    load.add_argument(
        ...
        dest='list',
        action='store_true',
        help='List all configs in $TMUXP_DIR (default is ~/.tmuxp).'
    )
...
def main():
    ...
    if tmuxp_config['warning'] != False:
        ...
        if tmuxp_config['warning'] is None:
            tmuxp_config['warning'] = False
    if not args.colors:
        args.colors = tmuxp_config['colors']
